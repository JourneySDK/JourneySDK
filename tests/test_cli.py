...
from journeysdk.state import PausedStepState, default_execution_state_path, load_execution_state
...
def test_parser_accepts_new_flags_and_rejects_removed_forms(
    ...
    assert verify_args.file == "journeys.py"
    assert verify_args.journey == "alpha"
    assert verify_args.step == "target"
    assert verify_args.output == "jsonl"
    ...
    assert default_discover_args.output == "source"

    step_discover_args = discover_parser.parse_args(
        ["open_main_page", "--file", "journeys/app_journey.py", "--journey", "app_journey"]
    )
    ...
    with pytest.raises(SystemExit):
        parser.parse_args(["--step", "target"])
    with pytest.raises(SystemExit):
        parser.parse_args(["--develop-step", "target"])
    with pytest.raises(SystemExit):
    ...
    with pytest.raises(SystemExit):
        discover_parser.parse_args(["http://127.0.0.1:3000", "--force"])
...
def _compiled_anchor_plan(case_plans: list[CasePlan]) -> _CompiledJourney:
    ...
    return _CompiledJourney(
        file_path=Path("journeys/app.py"),
        journey_name="app_journey",
        function=flow,
        plan=JourneyPlan(
            journey_id="app_journey",
            function_ref="tests.flow",
    ...
    case_a = CasePlan(
        ...
        nodes=[
            ...
            BranchMarkerNode(
                ...
                group_id="group",
                active_key="a",
                start_from="step_open_main_page",
            ),
        ],
    )
    case_b = CasePlan(
...
def test_discover_anchor_resolution_rejects_branch_specific_matches() -> None:
    case_a = CasePlan(
        ...
        nodes=[
            ...
            StepNode(
                node_id="step_open_settings_a",
                label="open_settings",
                fn_ref="tests.open_settings_a",
                args=(),
                kwargs={},
            ),
        ],
    ...
    case_b = CasePlan(
        ...
        nodes=[
            ...
            StepNode(
                node_id="step_open_settings_b",
                label="open_settings",
                fn_ref="tests.open_settings_b",
                args=(),
                kwargs={},
            ),
        ],
    ...
    assert len(errors) == 1
    assert "ambiguous" in errors[0].message
    assert "case_a" in (errors[0].hint or "")
...
def test_discover_anchor_execution_pauses_after_target_step(
    ...
    target_step = StepNode(
        node_id="step_open_checkout",
        label="open_checkout",
        fn_ref="tests.open_checkout",
        args=(),
        kwargs={},
    )
    ...
    def fake_execute_plan(*args: object, **kwargs: Any) -> _PausedExecution:
        del args
        ...
        return _PausedExecution(
            ...
            case_report=CaseExecutionReport(
                case_id="case_checkout",
                branch_env={},
                records=[
                    NodeExecutionRecord(
                        ...
                        label="open_checkout",
                        status="executed",
                        result={"workspace_id": "w-1"},
                        side_outputs={"browser_page": ("page",)},
                    )
                ],
                completed=False,
                stopped_at_label="open_checkout",
            ),
        )
...
def test_discover_source_output_prints_raw_source(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_discover_jsonl_output_includes_source(
    ...
    exit_code = main(["discover", "http://127.0.0.1:3000", "--output", "jsonl"])

    events = _jsonl_events(capsys.readouterr().out)
    result_events = [event for event in events if event["event"] == "discover_result"]
    assert exit_code == 0
    assert len(result_events) == 1
    assert result_events[0]["source"] == "from journeysdk import journey\n"
...
def test_discover_url_mode_rejects_file(
    ...
    exit_code = main(
        ["discover", "http://127.0.0.1:3000", "--file", "journeys/out.py"]
    )

    captured = capsys.readouterr()
    assert exit_code == 1
    assert captured.out == ""
    assert "generated code is printed to stdout; redirect stdout if a file is desired" in captured.err
...
def test_discover_step_mode_uses_anchor_page_state(
    ...
    exit_code = main(
        ["discover", "open_main_page", "--file", "journeys/app.py", "--journey", "app"]
    )

    captured = capsys.readouterr()
    assert exit_code == 0
    assert captured.out == "def discover_after_open_main_page(anchor_page):\n    pass\n"
...
def test_discover_step_mode_uses_browser_side_output_for_non_page_anchor(
    ...
    captured = capsys.readouterr()
    assert exit_code == 0
    assert "discover_after_prepare_configured_workspace" in captured.out
...
def test_help_outputs_include_agentic_command_manual():
    ...
    assert "browse Journey evidence from completed runs" in logs_help
    ...
    assert "Generated code is never written by this command" in discover_help
...
def test_touchpoint_docs_prints_reference_without_discovery(
    ...
    exit_code = main(["touchpoints", "docker"])
...
def test_touchpoint_docs_all_prints_index_and_references(
    ...
    exit_code = main(["touchpoints", "all"])
...
def test_execute_structured_output_is_rejected(
    ...
    with pytest.raises(SystemExit) as exc_info:
        main(["verify", "--reuse-state", "--file", "missing.py", "--output", "structured"])

    output = capsys.readouterr().out
    assert exc_info.value.code == 2
    assert "invalid choice: 'structured'" in output
    assert "What happened: journey verify: error: argument --output: invalid choice" in output
    assert "Try this:" in output
    assert "Next commands:" in output
...
def test_execute_forwards_no_browser_recording_flag(
    ...
    exit_code = main(
        ["verify", "--reuse-state", "--file", "flow.py", "--no-browser-recording", "--log-level", "off"]
    )
...
def test_execute_interactive_requires_develop_step(
    ...
    captured = capsys.readouterr()
    assert exc_info.value.code == 2
    assert "unrecognized arguments: --interactive" in captured.out
    assert captured.err == ""
...
def test_execute_output_jsonl_emits_parseable_log_events(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--reuse-state", "--file", "flow.py", "--output", "jsonl"])
...
def test_execute_discovers_decorated_journeys_recursively_and_via_aliases(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--reuse-state", "--output", "jsonl"])
...
def test_execute_prints_all_selected_plans_before_any_journey_runs(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--reuse-state"])
...
def test_execute_log_level_off_suppresses_diagnostics(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--reuse-state", "--file", "flow.py", "--log-level", "off"])
...
def test_execute_branch_anchor_status_visible_in_pretty_and_jsonl(
    ...
    pretty_exit = main(["verify", "--reuse-state", "--file", "flow.py"])
    ...
    jsonl_exit = main(["verify", "--reuse-state", "--file", "flow.py", "--output", "jsonl"])
...
def test_execute_file_and_journey_filters_limit_selection(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(
        ["verify", "--reuse-state", "--file", "pkg/first.py", "--journey", "alpha", "--output", "jsonl"]
    )
...
def test_execute_errors_when_journey_name_is_ambiguous(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--reuse-state", "--journey", "shared"])
...
def test_cli_renders_user_friendly_missing_file_error(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--reuse-state", "--file", "missing.py"])
...
def test_execute_step_runs_only_the_unique_matching_journey(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--step", "target", "--output", "jsonl"])
...
def test_execute_step_errors_when_label_is_ambiguous_across_journeys(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--step", "shared"])
...
def test_execute_output_jsonl_errors_include_hint_for_missing_step(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--step", "missing", "--file", "alpha.py", "--output", "jsonl"])
...
def test_execute_streams_live_case_progress_for_all_branches(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--reuse-state", "--file", "flow.py"])
...
def test_execute_step_streams_live_target_progress_and_replay_anchor(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--step", "finish_manual", "--file", "flow.py"])
...
def test_execute_continues_and_summarizes_compile_failures_by_default(
    ...
    assert "Next commands:" in output
    assert "journey verify --file broken.py --journey broken" in output
    removed_plan_inspection_flag = "--" + "debug" + "-plan"
...
def test_fail_fast_stops_before_later_journeys_are_processed(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--reuse-state", "--fail-fast"])
...
def test_execute_default_state_uses_single_file_after_success(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--reuse-state", "--log-level", "off"])
...
def test_execute_default_state_persists_but_reruns_after_completion(
    ...
    first_exit = main(["verify", "--reuse-state", "--file", "flow.py", "--log-level", "off", "--no-logs"])
    second_exit = main(["verify", "--reuse-state", "--file", "flow.py", "--log-level", "off", "--no-logs"])
...
def test_execute_no_state_skips_default_state_file(
    ...
    monkeypatch.chdir(tmp_path)
    exit_code = main(["verify", "--file", "flow.py", "--log-level", "off"])
...
def test_execute_no_state_update_reads_without_persisting_progress(
    ...
    second_exit = main(
        ["verify", "--reuse-state", "--file", "flow.py", "--no-state-update", "--log-level", "off"]
    )
...
def test_execute_state_interrupts_and_resumes_via_cli(
    ...
    first_exit = main(["verify", "--reuse-state", "--file", "flow.py"])
    first_output = capsys.readouterr().out
    ...
    second_exit = main(
        ["verify", "--reuse-state", "--file", "flow.py", "--output", "jsonl"]
    )
    payload = _execute_result_payload(capsys.readouterr().out)
...
def test_execute_state_first_sigint_finishes_step_and_resumes_after_it(
    ...
    second_exit = main(["verify", "--reuse-state", "--file", "flow.py"])
    second_capture = capsys.readouterr()
