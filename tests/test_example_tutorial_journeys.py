...
import example.branching_journey as branching_example
import example.cloud_webhook_journey as cloud_webhook_example
import example.fail_fast_journeys as fail_fast_example
import example.first_journey as first_example
import example.resume_journey as resume_example
import example.retry_journey as retry_example
import example.selection_journeys as selection_example
...
def test_first_journey_compiles_and_executes_in_order():
    first_example.reset_demo_state()

    plan = journey.compile_journey(first_example.first_journey)
    ...
    report = journey.execute(first_example.first_journey)

    assert [case.case_id for case in report.case_reports] == ["case_1"]
    assert first_example.EVENTS == [
...
def test_selection_journeys_compile_and_execute_independently():
    selection_example.reset_demo_state()

    welcome_plan = journey.compile_journey(selection_example.welcome_email_journey)
    invoice_plan = journey.compile_journey(selection_example.invoice_reminder_journey)
    ...
    welcome_report = journey.execute(selection_example.welcome_email_journey)
    invoice_report = journey.execute(selection_example.invoice_reminder_journey)
...
def test_branching_journey_compiles_and_targets_the_manual_path():
    branching_example.reset_demo_state()

    plan = journey.compile_journey(branching_example.branching_journey)
    ...
    report = journey.execute(
        branching_example.branching_journey,
    ...
    assert branching_example.EVENTS == [
...
def test_retry_examples_show_same_step_step_anchor_and_checkpoint_anchor():
    retry_example.reset_demo_state()
    journey.execute(retry_example.retry_current_step_journey)
    assert retry_example.EVENTS == [
    ...
    retry_example.reset_demo_state()
    journey.execute(retry_example.retry_from_step_result_journey)
    assert retry_example.EVENTS == [
    ...
    retry_example.reset_demo_state()
    journey.execute(retry_example.retry_from_checkpoint_journey)
    assert retry_example.EVENTS == [
...
def test_resume_example_resumes_from_saved_state(
    ...
    try:
        with pytest.raises(KeyboardInterrupt):
            journey.execute(resume_example.resume_journey, state=state_file)
    finally:
        stop_event.set()
    ...
    report = journey.execute(resume_example.resume_journey, state=state_file)
...
def test_cloud_webhook_example_compiles_and_executes(
    ...
    with serve_in_background() as cloud:
        ...
        plan = journey.compile_journey(cloud_webhook_example.cloud_webhook_journey)
        ...
        report = journey.execute(cloud_webhook_example.cloud_webhook_journey)

    assert [case.case_id for case in report.case_reports] == ["case_1"]
    assert cloud_webhook_example.EVENTS[-1] == "assert_invoice_paid_webhook"


def test_fail_fast_examples_include_one_success_and_one_expected_failure():
    report = journey.execute(fail_fast_example.good_demo_journey)

    assert [case.case_id for case in report.case_reports] == ["case_1"]

    with pytest.raises(CallableExecutionError) as exc_info:
        journey.execute(fail_fast_example.broken_demo_journey)
