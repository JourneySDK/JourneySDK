"""Private cloud client for the official email touchpoint."""
...
_LOGGER = get_logger("email-cloud")
...
def _request_json(
    *,
    ...
    _LOGGER.debug(
        "cloud_request_start",
        "calling Journey Cloud email API",
        method=method,
        route=route,
        url=url,
    )
    try:
        ...
        _LOGGER.error(
            ...
            "Journey Cloud email API returned an error",
            method=method,
            route=route,
            status=exc.code,
        )
        ...
        _LOGGER.error(
            ...
            "Journey Cloud email API was unreachable",
            method=method,
            route=route,
    ...
    if status == 204 and allow_no_content:
        _LOGGER.debug(
            ...
            "Journey Cloud email API returned no content",
            method=method,
            route=route,
            status=status,
        )
    ...
    if not raw_body:
        _LOGGER.debug(
            ...
            "Journey Cloud email API returned an empty JSON payload",
            method=method,
            route=route,
            status=status,
        )
    ...
    _LOGGER.debug(
        "cloud_request_success",
        "Journey Cloud email API returned JSON",
        method=method,
        route=route,
        status=status,
    )
