...
```bash
uv run journey verify --fresh --file docs/fail_fast_journeys/fail_fast_journeys.py
```
...
```bash
uv run journey verify --fresh --file docs/fail_fast_journeys/fail_fast_journeys.py --fail-fast
```
...
4. For browser failures, inspect the current URL/title, rendered-page artifacts from `journey dev --output jsonl`,
   screenshots, trace/video paths, and the app route or selector code before editing. The coding agent should use that
   context to update the Journey Python file directly, then rerun the focused step.
...
is expected after changes to the journey plan, step source, runtime, or workspace inputs. Use `--fresh` when you want
...
- With fresh verification, Ctrl-C is not resumable. The interrupted summary says to rerun with state updates enabled next time.
